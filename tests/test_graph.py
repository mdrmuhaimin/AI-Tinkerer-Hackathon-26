...
def test_missing_or_blank_name_falls_back_to_card_name(tmp_path: Path) -> None:
    image = _touch(tmp_path / "card.jpg")
    ...
    assert missing["contact_evidence"]["full_name"] == "Ada Lovelace"

    blank = graph.invoke(_pending(name="   ", image_path=image))
    assert blank["status"] == "complete"
    assert blank["contact_evidence"]["full_name"] == "Ada Lovelace"


def test_missing_name_without_card_name_is_invalid(tmp_path: Path) -> None:
    image = _touch(tmp_path / "card.jpg")
    fake = FakeExtractor({"company": "Analytical Engines"})
    graph = _graph(tmp_path, fake)
...
def test_stream_node_order_valid_and_invalid(tmp_path: Path) -> None:
    ...
    invalid_state = _pending(name="Ada Lovelace", image_path=None)
